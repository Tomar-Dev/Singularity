...
// YENİ: Auto-flush tamamen iptal edildi. Artık sadece bir bayrak.
void console_set_auto_flush(bool enabled) {
    (void)enabled; // Geriye dönük uyumluluk için tutuluyor ama işlevsiz.
}
...
// YENİ: Artık printf içinden VRAM'e yazmıyoruz. Sadece bayrak kaldırıyoruz.
void console_flush_if_needed() {
    __asm__ volatile("mfence" ::: "memory");
    // VRAM'e yazma işlemi artık sadece console_render_daemon tarafından yapılacak.
}
...
// YENİ: 60FPS Asenkron Çizim Görevi (Render Daemon)
void console_render_daemon() {
    while (1) {
        ...
        // 60 FPS = 16.6 ms. Bizim timer 250Hz (4ms). 4 tick = 16ms.
        timer_sleep(4); 
    }
}
