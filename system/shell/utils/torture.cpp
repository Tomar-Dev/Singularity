...
static void torture_math() {
    ...
    if (!A || !B || !C) {
        ...
        if(A) kfree_aligned(A);
        if(B) kfree_aligned(B);
        if(C) kfree_aligned(C);
        return;
    }
        ...
        for(int i=0; i<N_PTRS; i++) {
            if(get_rand() % 3 == 0) {
                if(ptrs[i] == nullptr) {
                    ...
                    ptrs[i] = kmalloc(sz);
                    if(ptrs[i]) memset(ptrs[i], 0xCC, sz);
                }
            }
        }
        for(int i=0; i<N_PTRS; i++) {
...
void run_torture_test() {
    ...
    for (int i = 0; i < target; i++) {
        ...
        if (i % 10 == 0) timer_sleep(1);
    }
    vga_set_color(VGA_LIGHT_GREEN, VGA_BLACK); printf("OK\n"); vga_set_color(VGA_WHITE, VGA_BLACK);
    ...
    while (true) {
        ...
        if (finished > last_finished) {
            ...
            stall_counter++;
            if (stall_counter == 50) {
                vga_set_color(VGA_LIGHT_RED, VGA_BLACK); printf("STALLED\n"); vga_set_color(VGA_WHITE, VGA_BLACK);
                break;
            }
        }
        timer_sleep(25); 
    ...
    if (dev) {
        ...
        if (buf) {
            ...
            vga_set_color(VGA_LIGHT_RED, VGA_BLACK); printf("OOM\n");
        }
    } else {
        vga_set_color(VGA_LIGHT_GREY, VGA_BLACK); printf("  > Skipped (No Disk).\n");
    }
