...
void kobject_ref(KObject* obj) {
    if (obj) {
        ...
        panic_at("handle.cpp", __LINE__, KERR_NULL_DEREFERENCE, "kobject_ref: NULL object!");
    }
}
...
// YENİ: Move Semantics (Sahiplik Transferi)
handle_t HandleTable::transfer(handle_t h) {
    uint32_t idx = h & 0xFFFFFFFF;
    uint16_t gen = (h >> 32) & 0xFFFF;
    if (idx == 0 || idx >= CAPACITY) return 0;
    ...
    KObject* obj = entry.obj;
    uint16_t caps = entry.caps;

    kobject_ref(obj); // Transfer sırasında objeyi hayatta tut

    // Eski Handle'ı kapat (Gönderen kişi artık kullanamaz)
    entry.obj = nullptr;
    entry.caps = 0;
    entry.generation++;
    ...
    // Yeni Handle oluştur (Alıcı için)
    handle_t new_h = alloc(obj, caps);
    kobject_unref(obj); // Geçici referansı düşür
