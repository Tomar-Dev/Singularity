...
uint16_t PCIeDevice::readWord(uint16_t offset) { return (uint16_t)((readDWord(offset) >> ((offset & 2) * 8)) & 0xFFFF); }
uint8_t PCIeDevice::readByte(uint16_t offset) { return (uint8_t)((readDWord(offset) >> ((offset & 3) * 8)) & 0xFF); }
...
void PCIeDevice::writeWord(uint16_t offset, uint16_t value) {
    uint32_t current = readDWord(offset);
    uint16_t shift = (offset & 2) * 8;
    uint32_t mask = 0xFFFF << shift;
    uint32_t newval = (current & ~mask) | ((uint32_t)value << shift);
    writeDWord(offset, newval);
}
...
namespace PCIe {
    ...
    PCIeDevice* getDevice(int index) {
        ...
        // OPTİMİZASYON YAMASI: Tek seferde (Value Copy) donanım verisi çekilir.
