...
// YENİ: PM Timer donanım kalibrasyonu için erişim portu (Getter)
...
static void* map_and_check_table(uint64_t phys_addr, const char* signature) {
    ...
    acpi_header_t* header = (acpi_header_t*)ioremap(phys_addr, sizeof(acpi_header_t), PAGE_PRESENT | PAGE_WRITE);
    ...
    return ioremap(phys_addr, len, PAGE_PRESENT | PAGE_WRITE);
}
...
// MİMARİ YAMASI: DSDT Brute-Force Spagetti kod ACPI Standartlarına göre "PackageOp" parse edecek hale getirildi.
static void parse_aml_package(uint8_t* ptr, uint16_t* typp_a, uint16_t* typp_b, bool* found_flag, uint32_t max_len) {
    ...
    if (bytes == 0) {
        ...
        if (max_len < (uint32_t)(bytes + 1)) return;
        ptr += (bytes + 1);
        max_len -= (bytes + 1);
    }
    ...
    for (int k = 0; k < num_elements && found_values < 2 && max_len > 0; k++) {
        ...
        if (is_val) {
            if (found_values == 0) { *typp_a = val << 10; found_values++; }
            else if (found_values == 1) { *typp_b = val << 10; found_values++; } else {}
        } else {}
    }
...
static int init_dsdt() {
    ...
    acpi_header_t* header = (acpi_header_t*)ioremap(dsdt_phys, sizeof(acpi_header_t), PAGE_PRESENT | PAGE_WRITE);
    if (!header) return -1;
    ...
    if (!sig_ok || len < 4) return -1;

    uint8_t* dsdt_data = (uint8_t*)ioremap(dsdt_phys, len, PAGE_PRESENT | PAGE_WRITE);
    if (!dsdt_data) return -1;
...
uint32_t acpi_remap_irq(uint8_t irq) {
    if (irq >= 16) return irq;
    return iso_remap[irq];
}
...
void init_acpi(void* multiboot_addr) {
    ...
    // MİMARİ YAMASI: Legacy BIOS bellek tarama fonksiyonu (EBDA & 0xE0000) 
    // tamamen silinmiştir. Sistem katı bir şekilde UEFI Multiboot zırhına geçirildi.
    if (!found_via_tag || rsdp_phys == 0) {
        ...
        rsdp = (rsdp_t*)ioremap(rsdp_phys, sizeof(rsdp_t), PAGE_PRESENT | PAGE_WRITE);
    }
    ...
    if (rsdp != NULL) {
        ...
        if (use_xsdt) {
            ...
            acpi_header_t* hdr = (acpi_header_t*)ioremap(xsdt_phys, sizeof(acpi_header_t), PAGE_PRESENT | PAGE_WRITE);
            if (hdr) {
                uint32_t len = hdr->length;
                iounmap(hdr, sizeof(acpi_header_t));
                
                if (len >= sizeof(acpi_header_t)) {
                    xsdt = (struct xsdt*)ioremap(xsdt_phys, len, PAGE_PRESENT | PAGE_WRITE);
                    if (!xsdt || !check_checksum((uint8_t*)xsdt, len)) {
                        if (xsdt) { iounmap(xsdt, len); xsdt = NULL; } else {}
                        use_xsdt = false;
                    } else {}
                } else {
                    use_xsdt = false;
        ...
        if (!use_xsdt) {
            ...
            acpi_header_t* hdr = (acpi_header_t*)ioremap(rsdt_phys, sizeof(acpi_header_t), PAGE_PRESENT | PAGE_WRITE);
            if (hdr) {
                uint32_t len = hdr->length;
                iounmap(hdr, sizeof(acpi_header_t));
                
                if (len >= sizeof(acpi_header_t)) {
                    rsdt = (struct rsdt*)ioremap(rsdt_phys, len, PAGE_PRESENT | PAGE_WRITE);
