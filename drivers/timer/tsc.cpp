...
extern "C" {
    ...
    uint64_t get_uptime_ns() {
        if (tsc_frequency == 0) { return 0; } else { /* Valid */ }
        uint64_t now = tsc_read_asm();
        if (now < boot_tsc) { return 0; } else { /* Valid */ }
        
        uint64_t delta = now - boot_tsc;
        uint64_t freq_mhz = tsc_frequency / 1000000;
        if (freq_mhz == 0) { return 0; } else { /* Valid */ }
    ...
    void sleep_ns(uint64_t ns) {
        if (tsc_frequency == 0) { return; } else { /* Valid */ }
