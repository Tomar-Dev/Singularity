...
#include "drivers/timer/pit_lock.h"
...
// FIX: Missing field `_pad` initializer resolved
...
void apic_enable_local() {
    ...
    if (!x2apic_mode && lapic_base_virt == 0) return;
...
void init_apic(uint32_t physical_addr) {
    ...
    sprintf(buf, "[APIC] Initialized. Phys: 0x%x\n", physical_addr);
    serial_write(buf);
}
...
void apic_timer_init(uint32_t frequency) {
    ...
    if (!timer_calibrated) {
        ...
        char buf[64];
        // NOLINTNEXTLINE(clang-analyzer-security.insecureAPI.DeprecatedOrUnsafeBufferHandling)
        sprintf(buf, "[APIC] Calibrated via PIT. Ticks/ms: %d\n", ticks_per_ms);
        serial_write(buf);
    }
