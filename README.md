...
*   **Zero-Overhead FFI:** Compiled using Clang/LLD with Cross-Language LTO. C++ and Rust code are merged at the LLVM IR level, eliminating FFI call overhead.
...
*   **Hardware Abstraction Layer (HAL):** Drivers are entirely decoupled from x86_64 specific assembly, paving the way for future ARM64/RISC-V ports.

### 2. Kernel Object Model (KOM) & ONS
*   **Everything is a KObject:** Replaces the traditional VFS. Devices, blobs (files), containers (directories), and events are all reference-counted objects.
*   **Object Namespace (ONS):** A native, lock-free hierarchical tree providing O(1) in-RAM caching and resolution.
*   **Zero-Copy DMA & Direct I/O:** Storage drivers use Extent Coalescing to map contiguous disk blocks directly to RAM, bypassing CPU memory copying completely.
...
*   **Xorriso & GRUB** (For ISO generation)
...
*   `make asm`: Generates a complete disassembly of the kernel for debugging.
