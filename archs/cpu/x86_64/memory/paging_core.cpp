...
void PagingManager::initPcid() {
    ...
    uint64_t cr4_val; 
    __asm__ volatile("mov %%cr4, %0" : "=r"(cr4_val));
    if (cr4_val & (1u << 17)) { pcid_enabled = true; return; }
    cr4_val |= (1u << 17);
    __asm__ volatile("mov %0, %%cr4" :: "r"(cr4_val));
    pcid_enabled = true;
}
...
uint16_t PagingManager::allocPcid() {
    if (!pcid_enabled) { return PCID_KERNEL; }
...
int PagingManager::mapPage(uint64_t virt, uint64_t phys, uint64_t flags) {
    ...
    PageTable* active_pml4 = reinterpret_cast<PageTable*>(p2v(current_cr3 & 0x000FFFFFFFFFF000ULL));

    uint16_t p4, p3, p2, p1; 
    getIndexes(virt, p4, p3, p2, p1);
    bool c4 = false, c3 = false;

    PageTable* pdp = active_pml4->createNextLevel(p4, PAGE_PRESENT | PAGE_WRITE | PAGE_USER, &c4);
    if (!pdp) { spinlock_release(&paging_lock, irq); return 0; }

    PageTable* pd = pdp->createNextLevel(p3, PAGE_PRESENT | PAGE_WRITE | PAGE_USER, &c3);
    if (!pd) {
        if (c4) { 
            uint64_t ph = active_pml4->entries[p4].getAddress(); 
            active_pml4->entries[p4].clear(); 
    ...
    if (!nx_enabled) { flags &= ~PAGE_NX; }
    if (!(flags & PAGE_USER) && virt >= KERNEL_SPACE_BASE) { flags |= PAGE_GLOBAL; }
...
int PagingManager::mapPage1G(uint64_t virt, uint64_t phys, uint64_t flags) {
    if (phys & (PAGE_SIZE_HUGE-1)) { return 0; }
    if (virt & (PAGE_SIZE_HUGE-1)) { return 0; }
...
int PagingManager::mapPagesBulk(uint64_t vs, uint64_t ps, size_t count, uint64_t flags) {
    ...
    auto rollback = [&](size_t done) {
        for (size_t j = 0; j < done; j++) {
            ...
            PageTable* qp = active_pml4->getNextLevel(q4); if(!qp) continue;
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        uint64_t ff = flags;
        if (!(flags & PAGE_USER) && v >= KERNEL_SPACE_BASE) { ff |= PAGE_GLOBAL; }
...
void PagingManager::unmapPage(uint64_t virt) {
    ...
    if(!pdp) { spinlock_release(&paging_lock, irq); return; }
    PageTable* pd = pdp->getNextLevel(p3);   
    if(!pd) { spinlock_release(&paging_lock, irq); return; }
    ...
    if(!pt) { spinlock_release(&paging_lock, irq); return; }
    pt->entries[p1].clear(); 
    spinlock_release(&paging_lock, irq);
    tlbInvalidate(virt, 1);
}
...
bool PagingManager::updatePageFlags(uint64_t virt, uint64_t af, uint64_t rf) {
    ...
    uint16_t p4, p3, p2, p1; 
    getIndexes(virt, p4, p3, p2, p1);
    PageTable* pdp = pml4->getNextLevel(p4); if(!pdp){spinlock_release(&paging_lock,irq);return false;}
    PageTable* pd = pdp->getNextLevel(p3);   if(!pd) {spinlock_release(&paging_lock,irq);return false;}
    ...
    PageTable* pt = pd->getNextLevel(p2); if(!pt){spinlock_release(&paging_lock,irq);return false;}
...
void PagingManager::checkTlbFlush(uint8_t cpu_id) {
    ...
    if (per_cpu_tlb_version[cpu_id] == cur) { return; }
...
bool PagingManager::handleCowFault(uint64_t fault_addr) {
    ...
    PageTable* pdp = active_pml4->getNextLevel(p4); if(!pdp){spinlock_release(&paging_lock,irq);return false;}
    PageTable* pd = pdp->getNextLevel(p3);   if(!pd) {spinlock_release(&paging_lock,irq);return false;}
    PageTable* pt = pd->getNextLevel(p2);    if(!pt) {spinlock_release(&paging_lock,irq);return false;}
    ...
    pdp = active_pml4->getNextLevel(p4); if(!pdp) goto abort;
...
bool PagingManager::handleDemandFault(uint64_t fault_addr) {
    ...
    e->setRaw(reinterpret_cast<uintptr_t>(frame) | flags);
