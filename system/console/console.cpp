...
// QUAL-001 FIX: Merkezi konsol paleti
...
static void draw_char_at_fast(char c, uint32_t x, uint32_t y, uint8_t fg_idx, uint8_t bg_idx) {
    if (!backbuffer) { return; } else { /* Valid */ }
...
static void draw_history_line(int logical_idx, uint32_t screen_y) {
    ...
    bool is_empty = false;
    if (!buffer_wrapped && logical_idx > (int)head_idx) { is_empty = true; } else { /* Present */ }
    if (!buffer_wrapped && logical_idx < 0) { is_empty = true; } else { /* Present */ }
...
static void console_redraw() {
    if (!history_buffer) { return; } else { /* Proceed */ }
...
static void erase_cursor() {
    if (!backbuffer || !history_buffer) { return; } else { /* Proceed */ }
...
static void draw_cursor() {
    if (!backbuffer || !cursor_visible || scroll_offset != 0 || !history_buffer) { return; } else { /* Render mode */ }
...
static void console_newline() {
    if (!history_buffer) { return; } else { /* Proceed */ }
    head_idx++;
    if (head_idx >= HISTORY_LINES) { head_idx = 0; buffer_wrapped = true; } else { /* Linear flow */ }
    ...
    if (scroll_offset == 0) {
        ...
        if (scroll_offset > max_history - lines_per_screen) { scroll_offset = max_history - lines_per_screen; } else { /* Keep bounds */ }
    ...
    if (auto_flush_enabled) { draw_cursor(); } else { /* Blocked */ }
}

static void console_putchar_internal(char c, uint8_t fg, uint8_t bg) {
    if (!backbuffer || !history_buffer) { return; } else { /* Valid */ }
    ...
    if (c == '\n') {
        if (auto_flush_enabled) { erase_cursor(); } else { /* Hidden */ }
        console_newline();
    } else if (c == '\b') {
        if (auto_flush_enabled) { erase_cursor(); } else { /* Hidden */ }
        if (cursor_x > 0) {
            ...
            if (cursor_y > 0) { cursor_y--; } else { /* Top */ }
            ...
            if (cursor_x < 0) { cursor_x = 0; } else { /* Validated */ }
        ...
        if (auto_flush_enabled) { draw_cursor(); } else { /* Ignored */ }
    } else if (c == '\t') { 
        cursor_x = (cursor_x + 4) & ~3; 
        if(auto_flush_enabled) { draw_cursor(); } else { /* Ignored */ }
    } else if (c == '\r') {
        if (auto_flush_enabled) { erase_cursor(); } else { /* Ignored */ }
        cursor_x = 0;
        if (auto_flush_enabled) { draw_cursor(); } else { /* Ignored */ }
    } else {
        ...
        cursor_x++;
        if (auto_flush_enabled) { draw_cursor(); } else { /* Wait for flush */ }
    }
}
...
void console_init() {
    if (!backbuffer) { return; } else { /* Framebuffer Ready */ } 
    if (!history_buffer) {
        history_buffer = (ConsoleChar(*)[MAX_COLS])kmalloc(HISTORY_LINES * MAX_COLS * sizeof(ConsoleChar));
        if (!history_buffer) { return; } else { /* Memory Acquired */ }
    } else {
        // Pre-initialized
    }
    ...
    if (chars_per_line > MAX_COLS) { chars_per_line = MAX_COLS; } else { /* Matched Size */ }
...
void console_set_auto_flush(bool enabled) {
    uint64_t flags = 0;
    if (!global_panic_active) { flags = spinlock_acquire(&console_lock); } else { /* Raw mode */ }
    ...
    if (!global_panic_active) { spinlock_release(&console_lock, flags); } else { /* Escaped lock */ }
...
void console_write_noflush(const char* str) {
    uint64_t flags = 0;
    if (!global_panic_active) { flags = spinlock_acquire(&console_lock); } else { /* Unlocked bypass */ }
    ...
    if (!global_panic_active) { spinlock_release(&console_lock, flags); } else { /* Unlocked bypass */ }
}
...
// LEG-001 FIX: vga_set_color Removed, Console Color ENUM structure established.
...
void console_clear() {
    uint64_t flags = 0;
    if (!global_panic_active) { flags = spinlock_acquire(&console_lock); } else { /* Bypassed */ }
    ...
    if (!global_panic_active) { spinlock_release(&console_lock, flags); } else { /* Free bypass */ }
}

void console_scroll_up() {
    uint64_t flags = 0;
    if (!global_panic_active) { flags = spinlock_acquire(&console_lock); } else { /* Free */ }
    
    erase_cursor();

    uint32_t total_written = buffer_wrapped ? HISTORY_LINES : (head_idx + 1);
    uint32_t max_scroll = 0;
    if (total_written > lines_per_screen) { max_scroll = total_written - lines_per_screen; } else { /* Max bounds hit */ }
    ...
    if (!global_panic_active) { spinlock_release(&console_lock, flags); } else { /* Bypassed */ }
...
void console_blink_cursor(bool state) {
    ...
    if (cursor_visible == state || !history_buffer) {
        if (!global_panic_active) { spinlock_release(&console_lock, flags); } else { /* Safe Drop */ }
        return;
    } else {
        // Switch state
    }
